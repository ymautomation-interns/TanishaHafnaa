        ...
        <div className="no-print w-56 overflow-y-auto shrink-0" style={{ background: PANEL, borderRight: "1px solid #382E52" }}>
          <SidebarSection label="📄 Templates" open={sections.templates} onToggle={() => toggleSection("templates")}>
            {Object.entries(TEMPLATES).map(([k, t]) => (
              <button key={k} onClick={() => applyTemplate(k)} className="w-full flex items-center gap-2 text-left text-xs px-2 py-1.5 rounded hover:bg-[#382E52]" style={{ color: "#D9D2EE" }}>
                <span className="flex rounded overflow-hidden shrink-0" style={{ width: 18, height: 14 }}>
                  {t.swatch.map((c, i) => <span key={i} style={{ background: c, width: "33.3%" }} />)}
                </span>
                {t.label}
              </button>
            ))}
